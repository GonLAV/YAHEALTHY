...
export const AchievementsPage = () => {
  ...
  const streaks = useMemo(() => {
    if (!data) return null;
    const consecutive = (has: (date: string) => boolean) => {
      let count = 0;
      for (let i = 0; i < 365; i++) {
        const d = addDays(today, -i);
        if (has(d)) count++;
        else if (i > 0 || !has(d)) break;
      }
      // Allow the streak to survive if today isn't logged yet but yesterday was
      if (count === 0) {
        for (let i = 1; i < 365; i++) {
          if (has(addDays(today, -i))) count++;
          else break;
        }
      }
      return count;
  ...
  const streakCards = [
    { key: 'food', icon: <Flame className="w-5 h-5" />, label: 'Food logging streak', value: streaks.food, goal: 7, unit: 'days', color: 'bg-amber-50 text-amber-600' },
    { key: 'hydration', icon: <Droplets className="w-5 h-5" />, label: 'Hydration streak', value: streaks.hydration, goal: 7, unit: 'days', color: 'bg-sky-50 text-sky-600' },
    { key: 'sleep', icon: <Moon className="w-5 h-5" />, label: 'Sleep tracking streak', value: streaks.sleep, goal: 7, unit: 'days', color: 'bg-indigo-50 text-indigo-600' },
    { key: 'weigh', icon: <Scale className="w-5 h-5" />, label: 'Weekly weigh-in', value: streaks.weighedThisWeek ? 1 : 0, goal: 1, unit: 'this week', color: 'bg-emerald-50 text-emerald-600' },
  ];
  ...
  return (
    <div className="animate-fade-up space-y-5">
      <div>
        <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 tracking-tight">Achievements</h1>
        <p className="text-sm text-slate-500 mt-0.5">Consistency compounds — celebrate the small wins</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {streakCards.map((s) => (
          <Card key={s.key} className="text-center">
            <span className={`w-12 h-12 rounded-2xl flex items-center justify-center mx-auto ${s.color}`}>{s.icon}</span>
            <p className="text-3xl font-extrabold text-slate-900 tabular mt-3">{s.value}</p>
      ...
      {/* Longest streak + server badges */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card title="Milestones" subtitle="Tap into your momentum">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {goals.map((g) => (
              <div
                key={g.id}
                className={`rounded-2xl border p-4 flex items-center gap-3 ${
                  g.earned ? 'border-emerald-100 bg-emerald-50/50' : 'border-slate-100 bg-slate-50/50'
                }`}
              >
                <span className={`text-2xl ${g.earned ? '' : 'grayscale opacity-40'}`}>{g.icon}</span>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-bold ${g.earned ? 'text-emerald-800' : 'text-slate-500'}`}>{g.name}</p>
        ...
        <Card title="Earned badges" subtitle={`${data.badges?.totalEarned || 0} from the server`}>
          {(data.badges?.badges || []).length === 0 ? (
            <div className="flex flex-col items-center text-center py-8">
              <span className="w-12 h-12 rounded-2xl bg-slate-50 text-slate-300 flex items-center justify-center">
                <Utensils className="w-6 h-6" />
              </span>
              <p className="text-sm text-slate-400 mt-3">Log food to earn your first badge.</p>
            </div>
          ) : (
            <div className="space-y-2.5">
              {(data.badges?.badges || []).map((b) => (
                <div key={b.id} className="flex items-center gap-3 rounded-xl border border-slate-100 px-4 py-3">
                  <span className="text-2xl">{b.icon}</span>
                  <div className="flex-1">
                    <p className="text-sm font-bold text-slate-800">{b.name}</p>
                    <p className="text-xs text-slate-400">{b.description}</p>
                  </div>
                  <Chip color="emerald">Earned</Chip>
                </div>
              ))}
            </div>
          )}
          {streaks.longestFood > 0 && (
            <p className="text-xs text-slate-400 mt-4">
              Your all-time longest logging streak: <span className="font-bold text-amber-600">{streaks.longestFood} days</span>
            </p>
          )}
        </Card>
      </div>
