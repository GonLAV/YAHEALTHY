...
const keyFor = (day: string, name: string, time: string) => `yahealthy.reminder.${day}.${name}.${time}`;
 ...
 */
export function useReminders(onMissed?: (message: string) => void) {
    ...
    const tick = async () => {
      ...
      try {
        ...
        for (const [name, times] of Object.entries(SCHEDULE)) {
          ...
          if (localStorage.getItem(keyFor(day, name, hhmm))) continue;
