...
// professional wrote and approved it. assertApproved() throws in production
// for an unapproved persona -- correct for that one persona's replies, but
// this call runs for both personas at module load, and index.js requires
// this file unconditionally at the top of its own module. An uncaught throw
// here doesn't block one persona, it fails require() itself and takes the
// whole app down with it -- confirmed live: with both personas unapproved,
// every route 500'd, not just chat. Caught here so the rest of the app (and
// the other persona, if it is approved) keeps running; the actual
// customer-facing block is enforced below, scoped to just the unapproved
// persona's replies -- see blockedReply().
function checkApproval(persona, promptPath) {
  try {
    return clinical.assertApproved(persona, promptPath);
  } catch (err) {
    console.error(`[clinical] ${err.message}`);
    return clinical.statusFor(persona, promptPath);
  }
}

const APPROVALS = {
  adi: checkApproval('adi', PROMPT_PATHS.adi),
  yoni: checkApproval('yoni', PROMPT_PATHS.yoni)
};

// What a customer gets instead of a real reply while their persona's prompt
// is unapproved in production. Not framed as a transient error ("try again
// in a moment") -- this can last hours or days until a professional actually
// reads the text, and repeating that to the same person texting back over
// time would look broken rather than explain what's actually going on. Not
// generated by the model and not "Adi/Yoni talking" any more than WELCOME in
// routes/whapi.js is, so the same one-emoji exception applies.
const UNAPPROVED_FALLBACK_HE =
  'היי! אנחנו כרגע משלימים בדיקה מקצועית לפני שממשיכים, ונחזור אליך בהודעה ברגע שזה מוכן. תודה על הסבלנות 🙏';

function blockedReply(activeBot) {
  return IS_PRODUCTION && !APPROVALS[activeBot]?.approved ? UNAPPROVED_FALLBACK_HE : null;
}
