import { FoodLog, HydrationLog, Recipe, SleepLog, Targets, WeightGoal, WeightLog } from '@/services/api';
...
function pickRecipe(data: CoachData, opts: { category?: string; maxCalories?: number }): Recipe | null {
  const pool = data.recipes.filter(
    (r) =>
      (!opts.category || r.category === opts.category) &&
      (!opts.maxCalories || r.calories <= opts.maxCalories)
  );
  if (pool.length === 0) return data.recipes[0] ?? null;
  return pool[Math.floor(Math.random() * pool.length)];
}

/** 3 personalized, data-driven daily recommendations */
export function dailyRecommendations(data: CoachData): Recommendation[] {
...
const DISCLAIMER =
  'This is general wellness guidance based on your logged data — not medical advice.';

/** Rule-based coach grounded in the user's real data. */
export function coachAnswer(question: string, data: CoachData): string {
  ...
  if (has(q, 'how am i', 'doing today', 'summary', 'how did i do')) {
    ...
    return `Here's your day so far: ${fmt(calories)} kcal logged${cal}, ${fmt(protein)}g protein${prot}, ${(water * 1000).toFixed(0)}ml of ${(data.waterGoal * 1000).toFixed(0)}ml water${
      lastSleep ? `, and ${lastSleep.sleep_hours}h of sleep last night` : ''
    }. ${
  ...
  if (has(q, 'dinner', 'eat for', 'meal under', 'what should i eat', 'suggest')) {
    ...
    return r
      ? `Try ${r.name} tonight — ${r.calories} kcal, ready in ${r.time_minutes} minutes. Ingredients: ${r.ingredients.join(', ')}. ${
          targetCal ? `That leaves about ${fmt(Math.max(0, targetCal - calories - r.calories))} kcal of headroom.` : ''
        }`
  ...
  if (has(q, 'weight', 'weigh')) {
    ...
    return `Your goal: ${data.goal.start_weight_kg}kg → ${data.goal.target_weight_kg}kg. Latest weigh-in: ${
      latest ? `${latest.weight_kg}kg` : 'none yet'
    }, ${dir} ${Math.abs(Math.round(change * 10) / 10)}kg from your start. ${
      data.weightLogs.length < 2 ? 'Log weekly weigh-ins to see a reliable trend.' : 'Keep weighing in consistently — the trend matters more than any single number.'
    }`;
  }

  if (has(q, 'calorie', 'kcal', 'calories')) {
    return targetCal
      ? `You've logged ${fmt(calories)} of your ${fmt(targetCal)} kcal target — ${fmt(Math.max(0, targetCal - calories))} kcal remaining. ${
          calories > targetCal ? 'Above target today; a lighter dinner balances it.' : 'Spread the rest across your remaining meals.'
        }`
  ...
  if (has(q, 'meal plan', 'tomorrow')) {
    ...
    return `For tomorrow: breakfast — ${b ? b.name : 'oats with fruit'}; lunch — ${l ? l.name : 'grilled chicken bowl'}; dinner — ${
      d ? d.name : 'fish + veggies'
    }. Head to the Meal Plan page to generate and save a full plan for the next days.`;
  }
